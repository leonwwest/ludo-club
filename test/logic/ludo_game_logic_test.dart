...
void main() {
  group('LudoGame logic', () {
    test('No moves when dice is null or zero', () {
      expect(LudoGame.getMovablePieces(_makeState()), isEmpty);
    ...
    test('Pieces can leave home only on a 6', () {
      ...
      final red =
          moved.newState.players.firstWhere((p) => p.color == PlayerColor.red);
    ...
    test('Captures when landing on opponent on non-safe field', () {
      ...
      final redPieces = List.generate(
          ...
          (i) => Piece(
              ...
              i,
              i == 0
                  ? const PiecePosition(1, isHome: false)
                  : const PiecePosition(GameState.basePosition)));
      final greenPieces = List.generate(
          ...
          (i) => Piece(
              ...
              i,
              i == 0
                  ? const PiecePosition(2, isHome: false)
                  : const PiecePosition(GameState.basePosition)));
      ...
      final result =
          LudoGame.movePiece(state, movable.firstWhere((p) => p.id == 0));
      expect(result.capturedOpponentPiece, isNotNull);

      final newGreen = result.newState.players
    ...
    test('Enters home stretch and can finish', () {
      ...
      final state2 = _makeState(
        ...
        players: [
          Player(id: 'p1', name: 'P1', color: PlayerColor.red, pieces: [
            Piece(PlayerColor.red, 0, const PiecePosition(5)),
            ...List.generate(
                3,
                (i) => Piece(PlayerColor.red, i + 1,
                    const PiecePosition(GameState.basePosition))),
      ...
      final finish =
          LudoGame.movePiece(state2, state2.currentPlayer.pieces.first);
      final redFinal =
          finish.newState.players.firstWhere((p) => p.color == PlayerColor.red);
    ...
    test('Winner is set when all pieces finished', () {
      ...
      final redPieces = [
        Piece(PlayerColor.red, 0, const PiecePosition(LudoGame.homePathLength),
            isSafe: true),
        Piece(PlayerColor.red, 1, const PiecePosition(LudoGame.homePathLength),
            isSafe: true),
        Piece(PlayerColor.red, 2, const PiecePosition(LudoGame.homePathLength),
            isSafe: true),
        Piece(PlayerColor.red, 3, const PiecePosition(5)),
      ];
      final state = _makeState(
        lastDice: 1,
        players: [
    ...
    test('Wrap-around on main path without home entry (blue)', () {
      ...
      final bluePieces = List.generate(
          4,
          (i) => Piece(
              PlayerColor.blue,
              i,
              i == 0
                  ? const PiecePosition(51, isHome: false)
                  : const PiecePosition(GameState.basePosition)));
      final state = _makeState(
      ...
      final blueAfter = result.newState.players
          .firstWhere((p) => p.color == PlayerColor.blue);
