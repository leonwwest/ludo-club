...
Piece _base(PlayerColor color, int id) =>
    Piece(color, id, const PiecePosition(-1));
  ...
  group('Move validation - home/goal rules', () {
    test('TC1: Red enters first home with exact 1 (allowed)', () {
      ...
      var state = _stateWith(
          players: [red, yellow], current: PlayerColor.red, lastDiceValue: 1);

      final movable = LudoGame.getMovablePieces(state);
      expect(
          movable.any((p) => p.color == PlayerColor.red && p.id == 0), isTrue);

      final res = LudoGame.movePiece(state, red.pieces.first);
      state = res.newState;
      final moved = state.players
    ...
    test('TC2: Yellow does not enter red home (stays on main path)', () {
      ...
      final yellowPiece =
          Piece(PlayerColor.yellow, 0, const PiecePosition(50, isHome: false));
      final yellow = _player(PlayerColor.yellow, [
        yellowPiece,
        _base(PlayerColor.yellow, 1),
        _base(PlayerColor.yellow, 2),
      ...
      final updatedYellow =
          res.newState.players.firstWhere((p) => p.color == PlayerColor.yellow);
    ...
    test('TC3: Red overshoots goal by 1 (invalid)', () {
      ...
      final green = _player(PlayerColor.green, [
        _base(PlayerColor.green, 0),
        _base(PlayerColor.green, 1),
        _base(PlayerColor.green, 2),
        _base(PlayerColor.green, 3)
      ]);
      final state = _stateWith(
          players: [red, green], current: PlayerColor.red, lastDiceValue: 2);
    ...
    test('TC4: Two reds can stack in home (no capture)', () {
      ...
      final red = _player(PlayerColor.red,
          [redA, redB, _base(PlayerColor.red, 2), _base(PlayerColor.red, 3)]);
      final blue = _player(PlayerColor.blue, [
        _base(PlayerColor.blue, 0),
        _base(PlayerColor.blue, 1),
        _base(PlayerColor.blue, 2),
        _base(PlayerColor.blue, 3)
      ]);
      final state = _stateWith(
          players: [red, blue], current: PlayerColor.red, lastDiceValue: 1);

      final res = LudoGame.movePiece(state, redB);
      final updatedRed =
          res.newState.players.firstWhere((p) => p.color == PlayerColor.red);
    ...
    test('TC5: Capture on main path works; none in home', () {
      final redPiece =
          Piece(PlayerColor.red, 0, const PiecePosition(10, isHome: false));
      final yellowPiece =
          Piece(PlayerColor.yellow, 0, const PiecePosition(11, isHome: false));
      final red = _player(PlayerColor.red, [
        redPiece,
        _base(PlayerColor.red, 1),
        _base(PlayerColor.red, 2),
      ...
      final updatedYellow =
          newState.players.firstWhere((p) => p.color == PlayerColor.yellow);
      ...
      final s2 = _stateWith(
        players: [
          _player(PlayerColor.red,
              [ra, rb, _base(PlayerColor.red, 2), _base(PlayerColor.red, 3)]),
          _player(PlayerColor.yellow, [
            _base(PlayerColor.yellow, 0),
            _base(PlayerColor.yellow, 1),
            _base(PlayerColor.yellow, 2),
            _base(PlayerColor.yellow, 3)
      ...
      final updRed =
          res2.newState.players.firstWhere((p) => p.color == PlayerColor.red);
